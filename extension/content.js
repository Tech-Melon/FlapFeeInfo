(() => {
  ...
  /** 缺参：等这一窗让同帧 host-fee 落地，再打 /modes（防首帧抖动） */
  const HOST_FEE_INCOMPLETE_DELAY_MS = 80;
  /** token → timeout：缺参 80ms 后立刻 /modes */
  const incompleteModesTimers = new Map();
  const hostListBootAt = Date.now();
  const HOT_BATCH_MIN_TOKENS = 2;
  /** Genius 组批窗：host-fee 一张张来时等这一窗再 POST，避免 1 条 wait_chain 串行。 */
  const GENIUS_BATCH_FLUSH_MS = 150;
  const GENIUS_BATCH_MIN_TOKENS = 2;
  ...
  // Token / K-line：顶栏先齐、侧栏约 0.9s 才挂卡，必须有 600/1200 补扫。
  const SPA_NAV_SCAN_OFFSETS_TOKEN_MS = [0, 80, 200, 600, 1200];
  const SPA_NAV_SCAN_OFFSETS_DEBOT_TOKEN_MS = [0, 80, 200, 600, 1200];
  ...
  const GMGN_ICON_QUOTE_RULES = [
    ...
    [/bianrensheng|bianren/i, "币安人生"],
    [/wbnb|bnbball|\bbnb\b/i, "BNB"]
  ];
  const GMGN_FEE_TOKEN_HREF_SELECTOR =
  ...
  function rememberPayoutSymbol(addr, sym) {
    const a = String(addr || "")
      .trim()
      .toLowerCase();
  ...
  function payoutContextIsRobinhood(ctx) {
    if (!ctx) return false;
    if (ctx === true) return true;
    if (typeof ctx === "object") {
      if (ctx.__pons_v2 === true) return true;
      if (ctx instanceof HTMLElement) {
        return isRobinhoodTokenRouteHref(readCardTokenHref(ctx));
      }
    }
    return false;
  }

  function symbolFromKnownPayoutAddress(addr, ctx) {
    const a = String(addr || "")
      .trim()
      .toLowerCase();
    if (!a) return "";
  ...
  /** 明确是 WBNB/零地址才当原生池；缺字段不等于 BNB（USDT 池同样没芯片）。 */
  function quoteTokenConfirmedNative(addr) {
    const a = String(addr || "")
      .trim()
      .toLowerCase();
    return a === WBNB_ADDRESS || a === "0x0000000000000000000000000000000000000000";
  }
  ...
  function dividendTokenIsConfirmedWbnb(entry) {
    const addr = String(entry?.dividend_token || entry?.top_payout_token || "")
      .trim()
      .toLowerCase();
    return addr === WBNB_ADDRESS;
  }
  ...
  function isGmgnPoolDomMismatch(card, icon, entry) {
    ...
    if (paintedPoolLooksNonNative(icon)) {
      if (want && quoteSymbolLooksNative(want)) {
        ...
        // 只有 DOM 明确写出 BNB 才允许从 USDT 退回；空 DOM 默认 BNB 不能覆盖。
        if (fromDom && isRealPoolQuoteSymbol(fromDom) && quoteSymbolLooksNative(fromDom)) {
  ...
  function clearBadgeAccessForLicense(reason) {
    ...
    incompleteModesTimers.forEach((id) => {
      try {
  ...
  function hostFeeQuoteReady(entry) {
    ...
    if (fromAddr && !quoteSymbolLooksNative(fromAddr)) return true;
    const qs = String(entry.quote_symbol || "").trim();
    if (qs && !quoteSymbolLooksNative(qs)) {
      if (isRealPoolQuoteSymbol(qs)) return true;
      if (/[\u4e00-\u9fff]/.test(qs)) return true;
      if (/^[A-Za-z0-9]{2,12}$/.test(qs)) return true;
    }
    // 有非零报价地址但还没有展示名（例如另一枚 7777 当底池）：等 /modes，不要只画 💎。
    return false;
  }
  ...
  function hostFeeCanSkipModes(entry) {
    ...
    if (hostFeeAllocationBps(entry) <= 0) return false;
    // /modes 还没落盘且报价地址不是原生、也没有展示名（另一枚 7777 当底池）→ 继续打。
    if (!hostFeeQuoteReady(entry) && !quoteTokenLooksNative(entry.quote_token || entry.quote_address || "")) {
      return false;
    }
    return true;
  }

  function cancelIncompleteModes(token) {
    const tok = String(token || "").toLowerCase();
    const timerId = incompleteModesTimers.get(tok);
    if (!timerId) return;
    try {
    ...
    incompleteModesTimers.delete(tok);
  }

  function kickHotLaneIfBusy() {
    if (!batchActive || hotLaneActive) return;
  ...
  const UNKNOWN_CACHE_MS = 6 * 60 * 60 * 1000;

  function fetchedAtMs(entry) {
    const t = Number(entry && entry.fetched_at) || 0;
    if (t <= 0) return 0;
  ...
  function tokenNeedsModesFetch(token) {
    ...
    if (isGeniusFunToken(tok)) {
      ...
      if (isFreshUnknown(gEntry, tok)) {
        ...
        if (gEntry.platform === "geniusfun" || gEntry.__geniusfun === true) return false;
        return true;
      }
  ...
  function overlayHasModesResult(entry) {
    if (!entry || isFeeLoadingEntry(entry) || isHostFeeEntryPending(entry)) return false;
    if (entry.source_host) return false;
    return true;
  }

  function overlayModesQueued() {
    if (searchOverlayModesTokens.size === 0) return false;
    for (const token of requestQueue) {
      if (searchOverlayModesTokens.has(token)) return true;
    }
    return false;
  }
  ...
  function queueSearchOverlayModes(token, href) {
    ...
    if (searchOverlayModesTokens.size >= 48) {
      ...
      if (oldest) searchOverlayModesTokens.delete(oldest);
    }
    searchOverlayModesTokens.add(tok);
    cancelIncompleteModes(tok);
  ...
  function forceModesForWaitingToken(tok) {
    ...
    if (searchOverlayModesTokens.has(token) || overlayModesQueued()) {
    ...
    kickHotLaneIfBusy();
    // Genius 绝不能 1 条 delayMs:0 立刷（js-mcp：整列变成串行 wait_chain，pending_map≈40）。
    if (isGeniusFunToken(token)) {
      maybeFlushRequestQueue("genius-force");
      return;
    }
    scheduleBatchFlush({ immediate: true, delayMs: 0 });
  }
  ...
  function scheduleIncompleteModes(tok) {
    ...
    if (releaseQueuedTokenIfHostFeeReady(token)) return;
    if (!tokenNeedsModesFetch(token)) return;
    if (incompleteModesTimers.size >= 48) {
      const oldest = incompleteModesTimers.keys().next().value;
      if (oldest) {
        try {
          window.clearTimeout(incompleteModesTimers.get(oldest));
        } catch (_old) {
          // ignore
        }
  ...
  function paintLoadingBadgeAndQueue(card, token, options = {}) {
    ...
    try {
      const keep = card.querySelector(`[${ICON_DATA}="1"]`);
      if (!(keep instanceof HTMLElement)) enforceIdentityOnCard(card);
    } catch (_id) {
      // ignore
    }
    try {
    ...
    try {
      ...
      if (existing) {
        try {
          applyBadgeUi(
            existing,
            computeBadgePresentation(FEE_LOADING_ENTRY, "", tok),
            tok
          );
        } catch (_ui) {
          // ignore
  ...
  let unpaintedSidebarCache = { at: 0, val: false };
  function hasUnpaintedGmgnSidebarTargets() {
    if (!isGmgnHost() || !isGmgnTokenPage()) return false;
    const now = Date.now();
    if (now - unpaintedSidebarCache.at < 400) return unpaintedSidebarCache.val;
    try {
      ...
      for (let ri = 0; ri < roots.length; ri += 1) {
        ...
        for (let i = 0; i < lim; i += 1) {
          ...
          if (!card.querySelector(`[${ICON_DATA}="1"]`)) {
            unpaintedSidebarCache = { at: now, val: true };
            return true;
          }
        }
      }
      if (sawRoot && (!sawCard || !sawViewportTarget)) {
        unpaintedSidebarCache = { at: now, val: true };
        return true;
      }
    } catch (_err) {
    ...
    unpaintedSidebarCache = { at: now, val: false };
    return false;
  }
  ...
  function paintUnpaintedTargetViewportQuick(reason, rootHint = null, bypassGap = false) {
    ...
    if (!bypassGap && isGmgnScrollCooling()) return 0;
    const now = Date.now();
    ...
    try {
      ...
      for (let i = 0; i < items.length; i += 1) {
        ...
        // 已有正确徽章才跳过；🪙BNCB 预览 / ⏳ 必须让 /modes 的 🎁 盖上去。
        try {
          ...
          if (good instanceof HTMLElement && isGmgnTrenchMisplacedBadge(card, good)) {
            ...
            const ent = getEntryForCard(card, ca) || resolveEntry(ca);
            const previewOnly =
              good.dataset.feeLoading === "1" ||
              (ent && (isHostFeeEntryPending(ent) || hostFeeStillNeedsModes(ent)));
  ...
  /** K 线内嵌战壕：与首页新卡同一套快补，不跟顶栏 settled / 4s enter freeze。 */
  let lastEmbeddedTrenchKickAt = 0;
  function kickEmbeddedTrenchBadges(reason) {
    if (!isTokenDetailRoute()) return;
    if (!isExtensionContextValid() || !isTabVisible()) return;
    if (!isAllowedScanChain()) return;
    const now = Date.now();
    const why = String(reason || "");
    if (why.includes("guardian") && now - lastEmbeddedTrenchKickAt < 800) return;
    lastEmbeddedTrenchKickAt = now;
    unpaintedSidebarCache = { at: 0, val: false };
    try {
    ...
    try {
      if (isGmgnHost()) {
        paintUnpaintedTargetViewportQuick(reason || "token-trench", null, true);
      } else if (isDebotHost()) {
        paintDebotUnpaintedViewportQuick(reason || "token-trench", null, true);
      }
    } catch (_paint) {
      // ignore
    }
  ...
  const TIP_I18N = {
    ...
    en: {
      ...
      burn: "Burn",
      lp: "LP",
      giggle: "Giggle charity",
      binance: "Binance charity",
      platform: "Platform"
    }
  };
  ...
  /** 搜索弹层未命中缓存的 token：立刻 /modes，不进战壕 80ms 缺参窗。 */
  const searchOverlayModesTokens = new Set();
  ...
  let tokenEnterExpireTimer = 0;
  let tokenEnterKickTimers = [];
  ...
  function scheduleGmgnOverlayPaint(reason, delayMs = 150, allowLateDomRetry = true) {
    ...
    gmgnOverlayPaintTimer = window.setTimeout(() => {
      ...
      const followSwap =
        /type-search/.test(String(reason || "")) || isDebotHost();
      if (allowLateDomRetry && (lastOverlayFastStats.seen === 0 || followSwap)) {
  ...
  /** Debot 战壕 href：BSC 全量 /token/bsc/（含 Genius 随机尾号）+ Flap/Four 尾号 + RH。 */
  const DEBOT_TRENCH_HREF_SEL =
    "a[href*='/token/bsc/'], [href*='/token/bsc/'], " +
    "[href*='/token/'][href*='8888'], [href*='/token/'][href*='7777'], " +
    "[href*='/token/'][href*='ffff'], a[href*='/token/robinhood/']";
  ...
  function isDebotTrenchRowCard(el) {
    ...
    const rh = isRobinhoodTokenRouteHref(href);
    if (rh) {
  ...
  /** GMGN 多链混选：localStorage chain-multi-select.selections.global；URL ?chain= 只是当前条。 */
  let gmgnChainSetCache = { at: 0, href: "", set: null };

  function readGmgnMultiSelectChains() {
    if (!isGmgnHost()) return [];
    try {
      const raw = localStorage.getItem("chain-multi-select");
      if (!raw) return [];
      const parsed = JSON.parse(raw);
      const g = parsed && parsed.selections && parsed.selections.global;
      if (!Array.isArray(g) || !g.length) return [];
      return g.map((s) => String(s || "").toLowerCase()).filter(Boolean);
    } catch (_err) {
      return [];
    }
  }

  function getActiveChainSet() {
    const href = String(location.href || "");
    const now = Date.now();
    if (
      gmgnChainSetCache.set &&
      gmgnChainSetCache.href === href &&
      now - gmgnChainSetCache.at < 400
    ) {
      return gmgnChainSetCache.set;
    }
    const set = new Set();
    for (const c of splitChainTokens(getUrlChain())) set.add(c);
    for (const c of readGmgnMultiSelectChains()) set.add(c);
    try {
    ...
    gmgnChainSetCache = { at: now, href, set };
    return set;
  }
  ...
  function rootHasRobinhoodTokenHref(root) {
    if (!(root instanceof HTMLElement) || !root.querySelector) return false;
    try {
      return Boolean(root.querySelector('[href*="/robinhood/token/"]'));
    } catch (_err) {
      return false;
    }
  }
  ...
  function isGmgnRobinhoodPage() {
    if (!isGmgnHost()) return false;
    if (/\/robinhood\/token\//i.test(location.pathname || "")) return true;
    if (isGmgnMixedChainPage()) return false;
    const set = getActiveChainSet();
    if (set.has("robinhood") && set.size === 1) return true;
  ...
  function isRobinhoodTokenRouteHref(href) {
    ...
    return (
      h.includes("/robinhood/token/") ||
      h.includes("/token/robinhood/") ||
      /[?&]chain=robinhood(?:&|$)/i.test(h)
    );
  }

  /** Debot/Gungnir 行：`/token/robinhood/0x…`，与 URL `?chain=` 无关 */
  function isDebotRobinhoodTokenHref(href) {
    return isDebotLikeHost() && isRobinhoodTokenRouteHref(href);
  }

  /** Debot 混合三列：BSC 税币路由或 Robinhood 代币路由都先收卡 */
  function isDebotMixedTrenchHref(href) {
  ...
  function rememberPonsSkipAddr(addr) {
    ...
    const now = Date.now();
    for (const [k, t] of ponsSkipAddrAt) {
      if (now - t > PONS_SKIP_TTL_MS) ponsSkipAddrAt.delete(k);
      if (ponsSkipAddrAt.size <= 300) break;
    }
    if (ponsSkipAddrAt.size > 400) {
      const first = ponsSkipAddrAt.keys().next().value;
      if (first) ponsSkipAddrAt.delete(first);
    }
  }

  function isPonsSkipAddr(addr) {
    const a = String(addr || "").toLowerCase();
    const t = ponsSkipAddrAt.get(a);
    if (!t) return false;
    if (Date.now() - t > PONS_SKIP_TTL_MS) {
      ponsSkipAddrAt.delete(a);
      return false;
    }
    return true;
  ...
  function rhHrefLooksSkipped(href) {
    if (!isRobinhoodTokenRouteHref(href)) return false;
  ...
  /**
   * URL 已是 SOL/ETH 等：即使 localStorage 混选含 BSC 也不当税币页（否则 SOL 战壕卡死）。
   * Debot `?chain=bsc` / `?chain=robinhood` 混排仍按卡 href 认链。
   */
  function urlIsExplicitNonFeeChain() {
    const path = String(location.pathname || "");
    if (
      /^\/(sol|eth|base|tron|monad|blast|aptos|sui|polygon|matic|avax)(\/|$)/i.test(path) &&
      !/\/bsc\/token\//i.test(path) &&
      !/\/robinhood\/token\//i.test(path)
    ) {
      return true;
    }
    const parts = splitChainTokens(getUrlChain());
    if (!parts.length) return false;
    if (
      parts.some(
        (c) => c === "bsc" || c === "robinhood" || c === "all" || c === "multi"
      )
    ) {
      return false;
    }
   ...
   * - Debot 双站: /meme?chain=bsc|robinhood（混合三列按卡 href）| /token/bsc/… | /token/robinhood/…
   */
  ...
  /** K 线顶栏 BaseInfoBar（约 70px）。禁止拿 main/body，否则 textContent 会扫图表+战壕卡死。 */
  function gmgnTokenHeaderScope() {
    try {
      const bar = document.querySelector('[data-sentry-source-file="BaseInfoBar.tsx"]');
      if (bar instanceof HTMLElement) {
        const r = bar.getBoundingClientRect();
        if (r.height > 0 && r.height <= 140) return bar;
      }
    } catch (_bar) {
      // ignore
    }
    ...
    if (addr instanceof HTMLElement) {
      let p = addr.parentElement;
      for (let i = 0; i < 8 && p && p !== document.body; i += 1) {
        if (p.getAttribute("data-sentry-source-file") === "BaseInfoBar.tsx") return p;
        try {
          const r = p.getBoundingClientRect();
          if (
            r.width > 400 &&
      ...
      return addr.parentElement instanceof HTMLElement ? addr.parentElement : addr;
    }
    return null;
  }
  ...
  function findGmgnTokenPageRoot() {
    ...
    const strip = gmgnTokenHeaderScope();
  ...
  function findGmgnTaxRateLabel() {
    ...
    let found = null;
    const root = gmgnTokenHeaderScope();
    if (root?.querySelectorAll) {
      ...
      for (let i = 0; i < max; i += 1) {
        ...
        if (t === "总税率" || (t.startsWith("总税率") && t.length <= 16)) {
          found = el;
          break;
        }
      }
  ...
  function findGmgnHeaderShortCaLeaf(urlTok) {
    ...
    try {
      ...
      // 2) Visible short-CA text leaves in header band（只扫顶栏，禁止整页）
      const headerScope = gmgnTokenHeaderScope() || document.querySelector("#token-base-address");
      const nodes = headerScope
        ? headerScope.querySelectorAll("span, a, div, p, button")
        : [];
  ...
  function tryPaintGmgnTokenHeader(reason) {
    ...
    if (
      ...
      if (entryHit && !isFeeLoadingEntry(entryHit)) {
        try {
          const q = headerPoolQuoteForPaint(
            urlTok,
            entryHit,
            headerCard instanceof HTMLElement ? headerCard : existingGood
          );
  ...
  function quickHasOpenOverlay() {
    ...
    try {
      ...
      if (!open && isDebotHost()) {
        ...
        for (let i = 0; i < Math.min(modals.length, 6); i += 1) {
          const el = modals[i];
          if (!(el instanceof HTMLElement) || isExplicitlyHiddenOverlay(el)) continue;
          const paper =
            el.querySelector?.('[role="dialog"], .MuiDialog-paper, .MuiPaper-root') || el;
          if (!(paper instanceof HTMLElement) || isExplicitlyHiddenOverlay(paper)) continue;
   ...
   */
  /**
   * Title/toolbar text only. Never read textContent of a virtual list — that
   * serializes every row and freezes the tab (收藏抽屉 / SOL 战壕).
   */
  function gmgnPanelChromeText(el) {
    if (!(el instanceof HTMLElement)) return "";
    const maxChars = 260;
    const chunks = [];
    const takeSmall = (n, depth) => {
      if (!(n instanceof HTMLElement) || depth > 3) return;
      if (chunks.join("").length >= maxChars) return;
      if (n.childElementCount > 12) {
        takeSmall(n.firstElementChild, depth + 1);
        const second = n.firstElementChild && n.firstElementChild.nextElementSibling;
        if (second && second.childElementCount <= 12) takeSmall(second, depth + 1);
        return;
      }
      const nodes = n.childNodes;
      for (let i = 0; i < nodes.length; i += 1) {
        const c = nodes[i];
        if (c.nodeType !== 3) continue;
        const t = String(c.nodeValue || "").replace(/\s+/g, "");
        if (!t) continue;
        chunks.push(t);
        if (chunks.join("").length >= maxChars) return;
      }
      let kid = n.firstElementChild;
      let k = 0;
      while (kid && k < 8 && chunks.join("").length < maxChars) {
        takeSmall(kid, depth + 1);
  ...
  function gmgnPanelSentryHay(el) {
    if (!(el instanceof HTMLElement)) return "";
    return `${el.getAttribute("data-sentry-source-file") || ""} ${
      el.getAttribute("data-sentry-component") || ""
    } ${String(el.className || "")}`;
  }
    ...
    try {
      ...
      for (let i = 0; i < 18 && p && p !== document.body; i++) {
        ...
        const structuralHint =
          GMGN_WALLET_TRACK_SENTRY_RE.test(sentry) ||
          /wallet|track|follow|monitor|watch/i.test(sentry);
  ...
  function clearTokenEnterKickTimers() {
    tokenEnterKickTimers.forEach((id) => window.clearTimeout(id));
    tokenEnterKickTimers = [];
    if (tokenEnterExpireTimer) {
      window.clearTimeout(tokenEnterExpireTimer);
      tokenEnterExpireTimer = 0;
    }
  }
  ...
  function armTokenEnterTransition() {
    ...
    clearTokenEnterKickTimers();
    const kickLater = (ms, why) => {
      const id = window.setTimeout(() => {
        tokenEnterKickTimers = tokenEnterKickTimers.filter((x) => x !== id);
        if (!isExtensionContextValid() || !isTabVisible()) return;
        if (!isTokenDetailRoute()) return;
        try {
          unpaintedSidebarCache = { at: 0, val: false };
      ...
      tokenEnterKickTimers.push(id);
    };
    kickLater(600, "token-enter-600");
    kickLater(1200, "token-enter-1200");
    tokenEnterExpireTimer = window.setTimeout(() => {
      tokenEnterExpireTimer = 0;
      finishTokenEnterTransition();
      if (!isExtensionContextValid() || !isTabVisible() || !isTokenDetailRoute()) return;
      kickEmbeddedTrenchBadges("token-enter-expire");
      scheduleScan(0, {
        force: false,
        immediate: false,
        light: false,
    ...
    try {
      ...
      for (let ri = 0; ri < scanRoots.length; ri += 1) {
        ...
        for (let i = 0; i < links.length; i += 1) {
          ...
          if (href && !isBscTokenRouteHref(href) && !isDebotRobinhoodTokenHref(href)) continue;
          const row =
  ...
  function scheduleScan(delay = 250, options = {}) {
    ...
    // 离场战壕冻结只挡首页扫描。K 线侧栏 PumpSub 一旦挂上就必须能扫。
    if (isTokenEnterTransitionActive()) {
      if (!isTokenDetailRoute()) return;
      if (
        isGmgnTokenPage() &&
        !document.querySelector(GMGN_FIXED_TRENCH_ROOT_SELECTOR)
      ) {
        return;
      }
    }
  ...
  function noteGmgnScrollActivity(scrollTarget = null) {
    ...
    // K 线图表/成交区 capture scroll 会把整页打进冷却，侧栏新卡被冻 0.6s+。
    // 只对战壕 PumpSub 列滚动冷却；图表滚动不挡新卡收集。
    if (isGmgnTokenPage()) {
      const el = scrollTarget instanceof HTMLElement ? scrollTarget : null;
      const inTrench = Boolean(el && el.closest?.(GMGN_FIXED_TRENCH_ROOT_SELECTOR));
      if (!inTrench) return;
    }
  ...
  function noteDebotScrollActivity(scrollTarget = null) {
    ...
    if (isDebotTokenPage()) {
      const el = scrollTarget instanceof HTMLElement ? scrollTarget : null;
      if (!el || !el.closest?.(".MuiCard-root, .MuiPaper-root, a[href*='/token/']")) {
        return;
      }
    }
  ...
  function collectGmgnNewCardMutations(records) {
    if (!isGmgnHost() || !records?.length || shouldDeferGmgnTrenchResizeWork()) return 0;
    if (isGmgnScrollCooling() && !isGmgnTokenPage()) return 0;
    const hrefSelector = GMGN_FEE_TOKEN_HREF_SELECTOR;
    const itemSelector = '[data-sentry-source-file="TokenItem.tsx"]';
    const seen = new Set();
    let discovered = 0;
    const tokenItemHasFeeHref = (el) => {
      if (!(el instanceof HTMLElement)) return false;
      try {
        if (el.matches?.(hrefSelector)) return true;
        return Boolean(el.querySelector?.(hrefSelector));
      } catch (_feeHref) {
        return false;
      }
    };
    const handleNode = (node) => {
      ...
      const candidates = [];
      if (node.matches(hrefSelector)) candidates.push(node);
      else if (node.matches(itemSelector) && tokenItemHasFeeHref(node)) {
        candidates.push(node);
      }
      if (node.childElementCount <= GMGN_NEW_CARD_LIMIT * 2) {
        node.querySelectorAll?.(hrefSelector).forEach((el) => {
    ...
    if (discovered > 0) {
      debugInfo("gmgn:new-card", { discovered });
    }
    return discovered;
  }
    ...
    const handleNode = (node) => {
      ...
      for (const candidate of candidates) {
        ...
        const token =
          ...
          normalizeToken(candidate.getAttribute("href") || "");
        if (!isFeeTargetToken(token || "", card)) continue;
        if (!cardHrefAllowedForScan(card) && !isDebotMixedTrenchHref(candidate.getAttribute("href") || "")) {
          continue;
        }
        seen.add(card);
        discovered += 1;
  ...
  function collectGmgnEmbeddedDirtyCards(records) {
    ...
    for (const record of records) {
      ...
      // 不要对 record.target（整列容器）querySelectorAll：价格跳动会把 K 线主线程打满。
      for (const node of record.addedNodes || []) {
  ...
  function gmgnMutationLooksRelevant(records) {
    ...
    const interestingHref = (href) => {
      ...
      if (h.includes("/bsc/token/") || h.includes("/robinhood/token/")) return true;
      return (
        h.includes("7777") ||
        h.includes("8888") ||
        h.includes("ffff") ||
    ...
    const probeEl = (el) => {
      ...
      // Shallow: only BSC/RH fee hrefs. Generic /token/ matches SOL and freezes.
      if (
        el.querySelector?.(
          `[${ICON_DATA}="1"], a[href*="/bsc/token/"], a[href*="/robinhood/token/"], a[href*="7777"], a[href*="8888"], a[href*="ffff"]`
        )
      ) {
        return true;
      }
  ...
  function debotMutationLooksRelevant(records) {
    ...
    const probeEl = (el) => {
      ...
      if (tag === "A") {
        ...
        if (
          /\/token\/bsc\//i.test(href) ||
          /\/token\/robinhood\//i.test(href) ||
          /(?:7777|8888|ffff)/i.test(href)
        ) {
          return true;
        }
      }
      if (el.querySelector?.(DEBOT_TRENCH_HREF_SEL)) {
        return true;
      }
    ...
    try {
      ...
      if (
        ...
        for (let i = 0; i < itemMax && out.length < cap; i += 1) {
          ...
          if (!(el instanceof HTMLElement)) continue;
          const href = el.getAttribute("href") || "";
          if (!isRobinhoodTokenRouteHref(href)) continue;
          const tok = extractAnyToken(href);
          if (!tok) continue;
  ...
  const trenchBadgeRaf = new WeakMap();
  function scheduleTrenchBadgePaint(card, fn) {
    if (!(card instanceof HTMLElement) || typeof fn !== "function") return;
    if (trenchBadgeRaf.has(card)) return;
    const id = window.requestAnimationFrame(() => {
      trenchBadgeRaf.delete(card);
   ...
   */
  function paintListCardFromCacheFast(card, token, entry, sync) {
    if (
      sync !== true &&
      card instanceof HTMLElement &&
      ((isGmgnHost() && isGmgnFixedTrenchCard(card)) ||
        (isDebotHost() && isDebotTrenchRowCard(card)))
    ) {
      scheduleTrenchBadgePaint(card, () =>
        paintListCardFromCacheFast(card, token, entry, true)
      );
      return true;
    }
    ...
    if (
      ...
      if (
        entry.__pons_v2 !== true &&
        (hostFeeAllocationBps(entry) > 0 || isGeniusFeeEntry(entry, token))
      ) {
    ...
    try {
      ...
      if (icon instanceof HTMLElement) {
        if (isGmgnTrenchMisplacedBadge(card, icon)) {
          removeAllBadgesForCard(card, icon.dataset.feeToken || token);
          icon = null;
        } else {
          applyBadgeUi(icon, presentation, token);
          return true;
      ...
      const ok = renderMode(card, token, entry, { forceRemount: false });
      if (ok) return true;
        ...
        if (!isGmgnTokenPage()) {
          ...
          if (!urlTok) {
            ...
            if (hasUnpaintedGmgnSidebarTargets()) {
              kickEmbeddedTrenchBadges("token-guardian");
              nextMs = 600;
            } else {
              nextMs = TOKEN_GUARDIAN_HEALTHY_MS;
            }
            ...
            if (!entry) {
              ...
              armGmgnHeaderDomWatch();
              kickEmbeddedTrenchBadges("token-guardian-header-miss");
    ...
    document.addEventListener(
      ...
      (event) => {
        try {
          ...
          const ph = String(t.getAttribute("placeholder") || "");
          if (
            name !== "new-search-input" &&
            !/搜索代币|搜名称|合约地址|钱包/.test(ph)
          ) {
            return;
          }
          if (
            !t.closest?.(".MuiModal-root, .MuiDialog-root, [role='dialog']") &&
            name !== "new-search-input" &&
            !quickHasOpenOverlay()
          ) {
            return;
          ...
          overlayFastUntil = Date.now() + OVERLAY_FAST_MS;
          overlayDetectCache = { at: 0, open: false };
          scheduleGmgnOverlayPaint("type-search", 0, true);
        } catch (_err) {
          // ignore
        }
      },
  ...
  /** Token page: header 齐且侧栏没有未画税卡，才停 progressive。 */
  function shouldCancelSpaProgressive() {
    ...
    try {
      if (quickHasOpenOverlay()) return false;
      if (isGmgnTokenPage()) {
        return hasGmgnTokenHeaderBadge() && !hasUnpaintedGmgnSidebarTargets();
      }
      if (isDebotTokenPage()) {
        return hasDebotTokenHeaderBadge();
      }
      return isTokenPageSettledWithBadge();
    } catch (_err) {
      return false;
    }
  }
  ...
  function beginSpaRouteSettle(prevKey, nextKey) {
    ...
    if (isGmgnTokenPage()) {
      ...
      window.setTimeout(() => kickEmbeddedTrenchBadges("token-enter"), 0);
      [600, 1200].forEach((ms) => {
        window.setTimeout(() => {
          if (!isExtensionContextValid() || !isTabVisible() || !isGmgnTokenPage()) return;
          kickEmbeddedTrenchBadges(`token-enter-${ms}`);
        }, ms);
      });
    ...
    offsets.forEach((offset, index) => {
      const timerId = window.setTimeout(() => {
        ...
        // Debot: 顶栏 + 侧栏战壕（不要等 header 齐了才扫列）。
        if (isDebotTokenPage()) {
          tryPaintDebotTokenHeader("spa-progressive");
          kickEmbeddedTrenchBadges("spa-token-trench");
          pendingLightScan = false;
        }

        // GMGN token: 顶栏 + 侧栏战壕，对齐首页新卡。
        if (isGmgnTokenPage()) {
        ...
        if (isDebotTokenPage()) {
          ...
          kickEmbeddedTrenchBadges("spa-token-trench-later");
  ...
  function forceAppendDebotHeaderBadge(header, token, entry) {
    ...
    try {
      const q = headerPoolQuoteForPaint(token, entry, header);
      const presentation = computeBadgePresentation(entry, q, token);
      const { label } = presentation;
      if (!label) return false;
  ...
  function isStablePaintedCard(card, forceRemount) {
    ...
    if (isGmgnHost()) {
      ...
      if (isGmgnHeaderMarkedCard(card) || isGmgnTokenHeaderCard(card)) {
        ...
        if (
          ...
          if (headerEntry && isHostFeeEntryPending(headerEntry)) return false;
          if (headerEntry && hostFeeStillNeedsModes(headerEntry)) return false;
          if (headerEntry && isGeniusFeeEntry(headerEntry, marked) && hostFeeAllocationBps(headerEntry) <= 0) {
            return false;
          }
    ...
    if (existing.dataset.feeLoading === "1") return false;
    const debotPending = resolveEntry(marked);
    if (debotPending && isHostFeeEntryPending(debotPending)) return false;
    if (debotPending && hostFeeStillNeedsModes(debotPending)) return false;
    if (
      isDebotHost() &&
  ...
  function scanVisibleCards() {
    ...
    const outerCards =
      listReturnSoft || isGmgnHost() || isDebotHost()
        ? allCards
  ...
  function getGmgnFixedSurfaceRoots() {
    ...
    const addTrenchRoot = (el) => {
      ...
      try {
        if (
          !el.querySelector?.(
            '[href*="/bsc/token/"], [href*="/robinhood/token/"]'
          )
        ) {
          return;
        }
  ...
  function getCandidateNodes() {
    ...
    const collectFromRoot = (root) => {
      ...
      if (gmgnLite && !listReturnSoft) {
        ...
        if (
          ...
          root.querySelectorAll('[data-sentry-source-file="TokenItem.tsx"]').forEach((n) => {
            const href = (n.getAttribute && n.getAttribute("href")) || "";
            if (!isRobinhoodTokenRouteHref(href)) return;
            if (rhHrefLooksSkipped(href)) return;
      ...
      const hnEarly = location.hostname || "";
      const debotHostEarly = hnEarly.endsWith("debot.ai") || hnEarly.endsWith("gungnir.bot");
      if (debotHostEarly && !overlayOnly && inView.length >= 16) return;
      ...
      // Debot 混合战壕 href 种子已够时禁止 350 节点 textContent 叶扫（强制布局）。
  ...
  function findCardsByCa(token) {
    ...
    try {
      ...
      forEachScoped(`[href*="${tok}"]`, (el) => {
        ...
        if (isGmgnHost()) {
          const item = el.closest?.('[data-sentry-source-file="TokenItem.tsx"]');
          if (item instanceof HTMLElement) card = item;
        } else if (isDebotHost()) {
          const climbed = climbDebotListCard(el);
          if (climbed instanceof HTMLElement) card = climbed;
        }
  ...
  function isDebotHotUnpaintedToken(token) {
    if (!isDebotHost() || !token) return false;
    try {
  ...
  function orderTokensForBatch(tokens) {
    ...
    for (let i = 0; i < tokens.length; i += 1) {
      ...
      try {
        ...
        const previewOnly =
          icon instanceof HTMLElement && icon.dataset.feeLoading === "1";
        if (!icon || previewOnly) score += 40;
        if (isGeniusFunToken(token) && (!icon || previewOnly)) score += 80;
        const cards = document.querySelectorAll(`[${CARD_DATA}="${token}"]`);
  ...
  function maybeFlushRequestQueue(_reason) {
    ...
    if (isGmgnHost() || isDebotHost()) {
      try {
        for (const t of requestQueue) {
          if (isGmgnHotUnpaintedToken(t) || isDebotHotUnpaintedToken(t)) {
            hot = true;
            noteGmgnHotWork();
            break;
          }
    ...
    const minTok = geniusQueued ? GENIUS_BATCH_MIN_TOKENS : hot ? HOT_BATCH_MIN_TOKENS : BATCH_MIN_TOKENS;
    const flushMs = geniusQueued ? GENIUS_BATCH_FLUSH_MS : hot ? HOT_BATCH_FLUSH_MS : BATCH_FLUSH_MS;
  ...
  function queueToken(token, options = {}) {
    ...
    if (options.forceModes !== true && !searchOverlayModesTokens.has(tok)) {
      ...
      if (!cachedHit && isPersistentCacheHit(tok) && !tokenNeedsModesFetch(tok)) return;
    }
    ...
    if (isGeniusFunToken(tok)) {
      let nGenius = 0;
      for (const t of requestQueue) {
        if (isGeniusFunToken(t)) nGenius += 1;
      }
      if (nGenius >= GENIUS_BATCH_MIN_TOKENS) {
        scheduleBatchFlush({ immediate: true, delayMs: 0 });
      } else {
        scheduleBatchFlush({ delayMs: GENIUS_BATCH_FLUSH_MS });
      }
      return;
    }
  ...
  /** /modes 回包：按徽章 data-fee-token 直接盖预览，不依赖 findCardsByCa / CARD_DATA。 */
  function applyModesResultToDom(token, entry) {
    const tok = String(token || "").toLowerCase();
    if (!tok || !entry) return 0;
    let n = 0;
    document.querySelectorAll(`[${ICON_DATA}="1"][data-fee-token="${tok}"]`).forEach((icon) => {
      if (!(icon instanceof HTMLElement) || !document.contains(icon)) return;
      try {
      ...
      let card = isGmgnHost()
        ? icon.closest?.('[data-sentry-source-file="TokenItem.tsx"]')
        : null;
      if (!(card instanceof HTMLElement) && isDebotHost()) {
        card = climbDebotListCard(icon);
      }
      if (!(card instanceof HTMLElement)) card = icon.parentElement;
      if (!(card instanceof HTMLElement)) return;
      try {
        card.dataset[CARD_MARK] = tok;
        card.setAttribute(CARD_DATA, tok);
      ...
      if (renderMode(card, tok, entry, { forceRemount: false })) n += 1;
    });
    return n;
  }

  /** 页面上还在 🪙BNCB loading 的卡，立刻 force /modes（刷新后 host-fee 预览的正路）。 */
  function queueVisiblePreviewModes(reason) {
    const seen = new Set();
    document.querySelectorAll(`[${ICON_DATA}="1"][data-fee-loading="1"]`).forEach((icon) => {
      if (!(icon instanceof HTMLElement)) return;
    ...
    if (seen.size) maybeFlushRequestQueue(reason || "preview-modes");
  }
  ...
  function processModesResponse(tokens, data, lane) {
    ...
    Object.entries(data.results || {}).forEach(([rawToken, result]) => {
      ...
      if (!entry) return;
      if (
        result.platform === "geniusfun" ||
        entry.__geniusfun ||
        entry.platform === "geniusfun" ||
        isGeniusFunToken(token)
      ) {
        rememberGeniusFunAddr(token);
      ...
      rememberPayoutSymbol(entry.quote_token, entry.quote_symbol);
    ...
    if (confirmed.length > 0) {
      ...
      confirmed.forEach(([token, entry]) => {
        ...
        applyModesResultToDom(token, entry);
        ingestFeeAndNotifyListHide(token, entry);
      });
   ...
   * GMGN / Debot 列表页共用。主批空闲时不启用（走正常单飞路径）。
   */
  async function flushHotLane() {
    ...
    for (const token of requestQueue) {
      ...
      if (
        isGmgnHotUnpaintedToken(token) ||
        isDebotHotUnpaintedToken(token) ||
        (isGeniusFunToken(token) && tokenNeedsModesFetch(token))
      ) {
        hotTokens.push(token);
        if (hotTokens.length >= HOT_LANE_MAX_TOKENS) break;
      }
    }
    const geniusHot = hotTokens.filter((t) => isGeniusFunToken(t));
    const sendHot = geniusHot.length ? geniusHot : hotTokens;
    if (sendHot.length === 0) return;
    sendHot.forEach((token) => {
      requestQueue.delete(token);
    });
    const controller = new AbortController();
    hotLaneAbortController = controller;
    const generation = (hotLaneGeneration += 1);
    hotLaneTokens = sendHot.slice();
    hotLaneActive = true;
    hotLaneStartedAt = Date.now();
    try {
      debugInfo("request:start", { tokens: sendHot, lane: "hot" });
      const data = await queryModes(sendHot, controller.signal);
      if (generation !== hotLaneGeneration) return;
      consecutiveFails = 0;
      processModesResponse(sendHot, data, "hot");
    } catch (error) {
      if (generation !== hotLaneGeneration) return;
      if (isContextInvalidError(error)) return;
      sendHot.forEach((token) => deferTokenRetry(token, "fail"));
      if (isAbortError(error)) {
        debugInfo("request:aborted", { tokens: sendHot, lane: "hot" });
      } else {
  ...
  async function flushTokenBatch() {
    ...
    // Genius 单独成批：和 Flap 混打时 Worker 会把 Genius miss 丢 waitUntil，预览卡 1min。
    const ordered = orderTokensForBatch(Array.from(requestQueue));
    const geniusOnly = ordered.filter((t) => isGeniusFunToken(t));
    const pick = geniusOnly.length ? geniusOnly : ordered;
    const cap = geniusOnly.length ? Math.min(24, MAX_BATCH_TOKENS) : MAX_BATCH_TOKENS;
    const tokens = pick.slice(0, cap);
  ...
  function isGeniusEmptyUnknown(entry) {
    if (!entry || entry.mode !== "unknown") return false;
    if (hostFeeAllocationBps(entry) > 0) return false;
    return (Number(entry.buy_tax_bps) || 0) + (Number(entry.sell_tax_bps) || 0) <= 0;
  }
  ...
  function normalizeResult(result) {
    ...
    return {
      ...
      platform_bps: Number(result.platform_bps) || 0,
      platform:
        typeof result.platform === "string" ? result.platform.trim().toLowerCase() : "",
      ...
      tax_symbol: typeof result.tax_symbol === "string" ? result.tax_symbol.trim() : "",
      dividend_symbol:
        typeof result.dividend_symbol === "string" ? result.dividend_symbol : "",
      dividend_token:
        typeof result.dividend_token === "string" ? result.dividend_token.toLowerCase() : "",
  ...
  /** 销毁箭头 = 本币名。不知道就空着，禁止用底池/分红（USDT）冒充。 */
  function burnPayoutSymbol(entry, quoteHint) {
    const tax = String((entry && entry.tax_symbol) || "").trim();
    const pay =
      entry && entry.top_segment === "burn"
        ? String(entry.top_payout_symbol || "").trim()
        : "";
    const src = tax || pay;
    if (!src) return "";
    const shown = tickerSymbolForArrow(src);
    if (!shown) return "";
    const q = compactDisplaySymbol(
      (entry && entry.quote_symbol) || quoteHint || ""
    );
    if (q && compactDisplaySymbol(shown) === q) return "";
    const div = compactDisplaySymbol((entry && entry.dividend_symbol) || "");
    if (div && compactDisplaySymbol(shown) === div && compactDisplaySymbol(tax) !== div) {
      return "";
    }
    return shown;
  }
  ...
  function ingestGmgnQuotesCatalog(json) {
    ...
    const chainKeys = new Set([chainKey, "bsc"]);
    if (getActiveChainSet().has("robinhood")) chainKeys.add("robinhood");
    const rows = [];
    chainKeys.forEach((k) => {
      if (Array.isArray(configs[k])) rows.push(...configs[k]);
    });
    let n = 0;
  ...
  function looksLikeStockQuoteChip(sym, entry) {
    ...
    // 只认本卡篮子/分红符号（虚拟列表残留内图），不靠全球股票名单。
    if (stockChipMatchesBasket(s, entry)) return true;
    const div = compactBasketSymbol(entry && entry.dividend_symbol);
    return Boolean(div && s === div);
  }

  function vaultDefaultPoolQuote(card) {
    if (siteStrategy.name === "gmgn") {
      const href = card ? readCardTokenHref(card) : "";
      if (isRobinhoodTokenRouteHref(href)) return "ETH";
   ...
   * 5) 无外图且地址为空/WBNB → GMGN 默认 BNB；Debot 战壕靠流动池 DOM，搜索等 /modes，不默认 BNB
   * 金库：Tax 内图是篮子，不当底池。
   * 已画出的 USDT/QQQB 不能被空 DOM 的默认 BNB 回退覆盖。
   */
  function pickStablePoolQuote(entry, card) {
    ...
    if (forceVaultNativePoolQuote(entry)) {
      ...
      return vaultDefaultPoolQuote(card);
    }

    const rhPool =
      (entry && entry.__pons_v2 === true) ||
    ...
    if (fromApi && isRealPoolQuoteSymbol(fromApi)) return formatPoolQuoteSymbol(fromApi);
    if (siteStrategy.name === "gmgn") {
      if (quoteTokenConfirmedNative(qTok)) {
        const href = card ? readCardTokenHref(card) : "";
        if (isRobinhoodTokenRouteHref(href)) return "ETH";
        if (isBscTokenRouteHref(href)) return "BNB";
        return GMGN_CHAIN_NATIVE_QUOTE[getGmgnChainKey()] || "BNB";
  ...
  /** K 线顶栏：Genius / 无报价时不要用 BNB 填空（0xfb9f 被画成 🪙BNB | 🎁→BNB）。 */
  function headerPoolQuoteForPaint(token, entry, host) {
    let q = "";
    if (host instanceof HTMLElement) q = resolveQuoteSymbol(host, entry);
    if (!q && entry) q = formatPoolQuoteSymbol(entry.quote_symbol || "") || "";
    if (q) return q;
    const tok = String(token || "").toLowerCase();
    if (isGeniusFeeEntry(entry, tok) || isGeniusFunToken(tok)) return "";
    if (TARGET_TOKEN_RE.test(tok)) return "BNB";
    return "";
  }
  ...
  function extractDebotPoolQuoteFromDom(card) {
    ...
    if (isRobinhoodTokenRouteHref(readCardTokenHref(card))) {
      const skipAlt = /^(robinhood|pons|pons v2|pons_v2)$/i;
      const imgs = card.querySelectorAll("img[alt]");
      for (let i = 0; i < imgs.length; i += 1) {
        const img = imgs[i];
        const alt = String(img.getAttribute("alt") || "").trim();
        if (!alt || skipAlt.test(alt)) continue;
        if ((img.width || 0) >= 40 && (img.height || 0) >= 40) continue;
        const fromAlt = normalizeQuoteSymbol(alt, { allowCjk: false });
        if (fromAlt && fromAlt.length <= 6) return fromAlt;
      }
    }
  ...
  function syncGmgnShareWorkerForFilters() {
    try {
      ...
      if (localStorage.getItem(ownKey) === "1") {
   ...
   * GMGN：改过滤条件后整页 reload，让 page-hook 用新规则滤首包。
  ...
  function normalizeCardMarkHexColor(raw, fallback) {
    const s = String(raw || "").trim();
    if (/^#[0-9a-fA-F]{6}$/.test(s)) return s.toLowerCase();
    if (/^#[0-9a-fA-F]{3}$/.test(s)) {
      return `#${s[1]}${s[1]}${s[2]}${s[2]}${s[3]}${s[3]}`.toLowerCase();
  ...
  function matchDevCountOp(count, op, min) {
    if (op === "lt") return count < min;
    if (op === "lte") return count <= min;
    if (op === "eq") return count === min;
    if (op === "gt") return count > min;
    return count >= min;
  }

  function normalizeDevCountMarkPrefs(raw) {
    ...
    for (let i = 0; i < list.length && out.rules.length < DEV_COUNT_MARK_MAX; i += 1) {
      ...
      out.rules.push({
        ...
        op,
        min,
        color: normalizeCardMarkHexColor(r.color, "#f59e0b"),
  ...
  function normalizeTwHandleMarkPrefs(raw) {
    ...
    for (let i = 0; i < list.length && out.rules.length < TW_HANDLE_MARK_MAX; i += 1) {
      const r = list[i] || {};
      const handle = normalizeCardMarkHandle(r.handle);
      if (!handle || handle.length < 2 || seen.has(handle)) continue;
      seen.add(handle);
      out.rules.push({
        id: String(r.id || `t${i}`),
        handle,
        note: String(r.note || "").trim().slice(0, 16),
        color: normalizeCardMarkHexColor(r.color, "#fbbf24"),
        enabled: r.enabled !== false
      });
    }
    return out;
  ...
  function cardMarkMetaAddr(raw) {
    const a = String(raw || "").trim().toLowerCase();
    return /^0x[a-f0-9]{40}$/.test(a) ? a : "";
  }

  function mergeCardMarkMetaEntries(entries) {
    if (!Array.isArray(entries) || !entries.length) return false;
    let changed = false;
    for (let i = 0; i < entries.length; i += 1) {
      const row = entries[i];
      const addr = cardMarkMetaAddr(row && row.address);
      if (!addr) continue;
      const prev = cardMarkMetaByAddr.get(addr);
      ...
      cardMarkMetaByAddr.set(addr, next);
      changed = true;
    }
    if (cardMarkMetaByAddr.size > 1200) {
      const keys = Array.from(cardMarkMetaByAddr.keys());
      for (let i = 0; i < keys.length - 800; i += 1) {
        cardMarkMetaByAddr.delete(keys[i]);
      }
    }
    return changed;
  }

  function formatDevCountChip(n) {
    if (n >= 10000) {
      const k = n / 1000;
  ...
  function pickDevCountRule(count) {
    ...
    for (let i = 0; i < rules.length; i += 1) {
      ...
      if (matchDevCountOp(n, r.op || "lt", r.min)) hits.push(r);
    }
    if (!hits.length) return null;
    for (let i = 0; i < hits.length; i += 1) {
      if (hits[i].op === "eq") return hits[i];
    }
    hits.sort((a, b) => {
      const spec = (r) => {
        if (r.op === "gt" || r.op === "gte") return r.min;
        if (r.op === "lt" || r.op === "lte") return -r.min;
        return 0;
      };
  ...
  function pickTwHandleRule(handle) {
    if (!twHandleMarkPrefs.enabled) return null;
    const h = normalizeCardMarkHandle(handle);
    if (!h) return null;
    const rules = twHandleMarkPrefs.rules || [];
    let truncated = null;
    for (let i = 0; i < rules.length; i += 1) {
      const r = rules[i];
      if (r.enabled === false) continue;
      if (h === r.handle) return r;
      if (h.length >= 4 && r.handle.startsWith(h)) {
        if (!truncated || r.handle.length > truncated.handle.length) truncated = r;
      }
    }
    return truncated;
  }
  ...
  function trenchMarkCardSelector() {
    return isGmgnHost()
      ? '[data-sentry-source-file="TokenItem.tsx"]'
      : 'a[href*="/token/bsc/"], a[href*="/token/robinhood/"]';
  }
  ...
  function scrapeCardMarkFromFiber(root) {
    let f = reactFiberOfCard(root);
  ...
  function resolveDebotMarkCard(el) {
    ...
    if (!/\/token\/bsc\//i.test(href) && !isBscTokenRouteHref(href) && !isRobinhoodTokenRouteHref(href)) return null;
  ...
  function clearCardMark(card) {
    if (!(card instanceof HTMLElement)) return;
    card.classList.remove("flap-card-mark", "flap-card-mark--dev", "flap-card-mark--tw");
    card.style.removeProperty("--flap-dev-color");
    card.style.removeProperty("--flap-tw-color");
  ...
  function clearAllCardMarks() {
    try {
      document.querySelectorAll(".flap-card-mark, [data-flap-mark-ca]").forEach((el) => {
        if (el instanceof HTMLElement) clearCardMark(el);
      });
    } catch (_all) {
      // ignore
    }
  ...
  function upsertMarkChip(card, kind, text, color, anchor) {
    ...
    let chip = card.querySelector(`[${attr}="1"]`);
    if (!text) {
      if (chip) chip.remove();
      return;
    }
  ...
  function chipInkForHex(hex) {
    const s = String(hex || "");
    if (!/^#[0-9a-fA-F]{6}$/.test(s)) return "#111";
    const n = parseInt(s.slice(1), 16);
    const r = (n >> 16) & 255;
    const g = (n >> 8) & 255;
    const b = n & 255;
    return 0.2126 * r + 0.7152 * g + 0.0722 * b > 148 ? "#111" : "#fff";
  }

  function applyCardMarkToEl(card, token) {
    if (!(card instanceof HTMLElement) || !token) {
      if (card instanceof HTMLElement) clearCardMark(card);
      return;
    }
    const addr = String(token).toLowerCase();
    ...
    card.classList.add("flap-card-mark");
    card.classList.toggle("flap-card-mark--dev", Boolean(devRule));
    card.classList.toggle("flap-card-mark--tw", Boolean(twRule));
    if (devRule) card.style.setProperty("--flap-dev-color", devRule.color);
    else card.style.removeProperty("--flap-dev-color");
    if (twRule) card.style.setProperty("--flap-tw-color", twRule.color);
    else card.style.removeProperty("--flap-tw-color");
    card.setAttribute("data-flap-mark-ca", addr);
    upsertMarkChip(
      card,
      "dev",
      devRule ? formatDevCountChip(count) : "",
  ...
  function hrefTokenFromMarkCard(card) {
    ...
    const href =
      card.getAttribute("href") ||
      card.querySelector?.("[href*='/bsc/token/0x'], [href*='/robinhood/token/0x'], [href*='/token/bsc/0x'], [href*='/token/robinhood/0x']")?.getAttribute("href") ||
      "";
  ...
  function applyCardMarksFromMutations(records) {
    ...
    for (let i = 0; i < records.length && seen.size < 24; i += 1) {
      ...
      for (let j = 0; j < added.length && seen.size < 24; j += 1) consider(added[j]);
    }
  }
  ...
  function applyGmgnNewCreationListHide() {
    // GMGN 虚拟列表禁止 DOM hide/reflow：按 children 重写 translateY 会打乱新创建顺序，
    // K 线还会把整列 height:0。过滤只走 page-hook 数据层。
  ...
  function hostFeeDividendBecameReal(prev, entry) {
    ...
    if (dividendPayoutLooksNative(entry)) return false;
  ...
  function applyHostFeeEntries(entries) {
    ...
    for (const raw of entries) {
      ...
      const top_payout_symbol =
        derived.top_segment === "burn"
          ? tax_symbol
          : String(raw.top_payout_symbol || raw.dividend_symbol || "").trim();
      const payload = {
        mode: derived.mode,
        label: "",
        title: "",
        ...
        dividend_token: String(raw.dividend_token || "").toLowerCase(),
      ...
      if (prev) {
        ...
        if (!dividendPayoutLooksNative(prev) && dividendPayoutLooksNative(entry)) {
          entry.dividend_symbol = prev.dividend_symbol;
          entry.dividend_token = prev.dividend_token || entry.dividend_token;
          entry.top_payout_symbol = prev.top_payout_symbol || entry.top_payout_symbol;
          entry.top_payout_token = prev.top_payout_token || entry.top_payout_token;
        }
      }
      modeCache.set(token, entry);
      // 齐套快画；缺参等 80ms 再 /modes（防同帧补齐抖动）。
      if (!hostFeeCanSkipModes(entry)) {
        if (searchOverlayModesTokens.has(token)) {
          queueSearchOverlayModes(token);
        } else {
          scheduleIncompleteModes(token);
        }
    ...
    if (isGmgnHost()) {
      ...
      try {
        if (isDebotTokenPage()) kickEmbeddedTrenchBadges("host-fee");
        else paintDebotHostFeeViewport("host-fee");
      } catch (_dp) {
        // ignore
      }
    }
    scheduleTaxRecvHideApply(30);
    queueVisiblePreviewModes("host-fee-preview");
  ...
  function bpsToPercentLabel(bps) {
    ...
    const v = n / 100;
    const decimals = n % 10 === 0 ? 1 : 2;
    return `${String(v.toFixed(decimals)).replace(/0+$/, "").replace(/\.$/, "")}%`;
  }

  function geniusShareToTokenBps(shareBps, entry) {
    const tax = Number(entry && entry.buy_tax_bps) || 0;
    const share = Number(shareBps) || 0;
    if (share <= 0) return 0;
    if (tax <= 0) return share;
    return Math.round((tax * share) / 10000);
  }

  function collectTipSegments(entry, token) {
    if (!entry) return [];
    const rows = [];
    const genius = isGeniusFeeEntry(entry, token);
    const push = (emoji, key, bps) => {
      const n = Number(bps) || 0;
      if (n <= 0) return;
      const shown = genius ? geniusShareToTokenBps(n, entry) : n;
      if (shown <= 0) return;
      rows.push({
        emoji,
        key,
        text: `${emoji} ${tipT(key)}`,
        pct: bpsToPercentLabel(shown)
      });
    };
    if (genius) {
      push(GIFT_EMOJI, "gift", effectiveGeniusGiftBps(entry));
      push("👨‍🍳", "creator", entry.market_bps);
      push("🔥", "burn", entry.deflation_bps);
      push("💧", "lp", entry.lp_bps);
      const plat = Number(entry.platform_bps);
      push("🪙", "platform", plat > 0 ? plat : 2500);
      return rows;
    }
    push("💎", "holder", entry.dividend_bps);
    const gift = Number(entry.gift_bps) || 0;
    if (gift > 0) {
      push(GIFT_EMOJI, "gift", gift);
      push("👨‍🍳", "creator", entry.market_bps);
    } else if (entry.is_vault) {
      push(GIFT_EMOJI, "gift", entry.market_bps);
    } else {
      push("👨‍🍳", "creator", entry.market_bps);
    }
    push("🎓", "giggle", entry.giggle_charity_bps);
    push("💛", "binance", entry.binance_charity_bps);
    push("🔥", "burn", entry.deflation_bps);
    push("💧", "lp", entry.lp_bps);
    return rows;
  }
  ...
  function buildTipModel(entry, quoteSymbol, label, token) {
    ...
    return {
      ...
      segments: collectTipSegments(entry, token),
  ...
  function computeBadgePresentation(entry, quoteSymbol, token) {
    ...
    const awaitChain =
      ...
      hostFeeAllocationBps(entry) <= 0 &&
      (Boolean(entry.source_host) || entry.__needsChain === true);
    return {
      label,
      title,
      className,
  ...
  function renderFeeTooltipContent(model) {
    ...
    if (segs.length) {
      ...
      secTitle.textContent = tipT("taxAlloc");
      sec.appendChild(secTitle);
      for (const row of segs) {
        appendTipKv(sec, row.text, row.pct);
      }
          ...
          if (entries && typeof entries === "object") {
            ...
            Object.entries(entries).forEach(([token, value]) => {
              ...
              persistentCache.set(token, {
                ...
                gift_bps: Number(value.gift_bps) || 0,
              ...
              if (value.platform === "geniusfun" || value.__geniusfun === true) {
                rememberGeniusFunAddr(token);
  ...
  function persistConfirmedModes(entries) {
    for (const [token, entry] of entries) {
      ...
      if (isGeniusFeeEntry(entry, token) && isGeniusEmptyUnknown(entry)) continue;
      persistentCache.set(token, {
        mode: entry.mode,
        label: entry.label,
        title: entry.title,
        ...
        platform: entry.platform || (entry.__geniusfun ? "geniusfun" : ""),
        __geniusfun: entry.__geniusfun === true,
        fetchedAt:
  ...
  const mutationObserver = new MutationObserver((records) => {
    ...
    if (
      ...
      if (gmgnDiscoveryMutationLooksRelevant(records)) {
        ...
        if (isGmgnTokenPage()) collectGmgnNewCardMutations(records);
      }
      return;
    }
    if (
      ...
      if (debotDiscoveryMutationLooksRelevant(records)) {
        ...
        if (isDebotTokenPage()) collectDebotNewCardMutations(records);
      }
      return;
    }
    ...
    // K 线图表滚动不得冻侧栏新卡收集。
    if (!isTokenDetailRoute() && (isGmgnScrollCooling() || isDebotScrollCooling())) {
      try {
    ...
    // 刷新首屏让宿主先画；首扫已按 GMGN_FIRST_SCAN_DELAY_MS 排队。新卡收集已在上面做完。
    if (
      isGmgnHost() &&
      !isTokenDetailRoute() &&
    ...
    // 0.4.43 GMGN: ignore pure ticker / unrelated node churn.
    if (isGmgnHost()) {
  ...
  if (isGmgnHost()) {
    if (isGmgnTokenPage()) {
      ...
      window.setTimeout(() => {
        try {
          tryPaintGmgnTokenHeader("boot-0");
        } catch (_err) {
          // ignore
        }
        kickEmbeddedTrenchBadges("boot-trench-0");
      }, 0);
      [80, 200, 600, 1200].forEach((ms) => {
        window.setTimeout(() => {
          if (!isExtensionContextValid() || !isTabVisible() || !isGmgnTokenPage()) return;
          try {
            tryPaintGmgnTokenHeader(`boot-${ms}`);
          } catch (_hdr) {
            // ignore
          }
          kickEmbeddedTrenchBadges(`boot-trench-${ms}`);
        }, ms);
      });
    } else {
